...
async function delay(ms) {
    ...
    await new Promise((resolve, reject) => {
        if (runningSearch == false) {
            // console.log("Delay stopped at " + new Date().toLocaleString());
            resolve();
        } else {
            setTimeout(() => {
                const end = Date.now();
                // console.log("Delayed for " + ms + "ms and completed at " + new Date().toLocaleString());
                resolve(end - start);
            }, ms);
        }
    })
...
async function clearSiteData() {
    ...
    return new Promise((resolve, reject) => {
        app.browsingData.remove(dataToRemove, {
            cacheStorage: true,
            cookies: true,
            localStorage: true,
            pluginData: true,
            ...
            console.log("Site data cleared at " + new Date().toLocaleString());
            resolve();
        });
    });
}
...
async function fetchStorage() {
    ...
    return new Promise((resolve, reject) => {
        app.storage.local.get([
            ...
            phoneUserAgent = result.phoneUserAgent || "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/14.0 Mobile/15E148 Safari/604.1";
            phoneWidth = parseInt(result.phoneWidth) || 390;
            phoneHeight = parseInt(result.phoneHeight) || 844;
            phoneDevicePixelRatio = parseFloat(result.phoneDevicePixelRatio) || 3;
            runningSearch = result.runningSearch || false;
            niche = result.niche || "random";
